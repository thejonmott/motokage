  ...
  const runInterrogation = async (query: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: query,
        config: { systemInstruction: `Mode: ${accessLevel}. You are Jonathan Mott's digital twin in the Motokage Studio calibration lab. Respond with high fidelity and professional precision.` }
      });
      ...
      <div className={`flex-grow flex flex-col bg-slate-900 border rounded-[3.5rem] overflow-hidden shadow-2xl relative transition-colors duration-500 ${accessLevel === 'CORE' ? 'border-purple-500/30' : 'border-indigo-500/20'}`}>
        <div className="px-12 py-8 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 backdrop-blur-md">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h3 className="text-xs font-bold text-white uppercase tracking-[0.4em]">Studio Calibration: {accessLevel}</h3>
              <p className="text-[8px] font-mono text-slate-500 uppercase tracking-widest mt-1">Status: Ready for Real-time Alignment</p>
            </div>
          </div>
        </div>
          ...
          {testLog.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-8 opacity-20 grayscale">
              ...
              <div className="max-w-xs space-y-3">
                 <h4 className="text-[10px] font-bold text-white uppercase tracking-widest">No Active Lab Sessions</h4>
                 <p className="text-[9px] text-slate-500 uppercase font-mono leading-relaxed">Interrogate the twin's reasoning to verify high-fidelity alignment in the Studio.</p>
              </div>
            </div>
          ) : (
            testLog.map((entry, i) => (
        ...
        <form onSubmit={(e) => { e.preventDefault(); runInterrogation(input); }} className="p-12 bg-slate-950/50 backdrop-blur-md border-t border-slate-800">
           <div className="flex gap-6 max-w-4xl mx-auto">
              <input 
                ...
                placeholder="Interrogate Studio reasoning..." 
        ...
        {isPromoting ? (
          <div className="bg-slate-900 border border-indigo-500/30 rounded-[3.5rem] p-10 space-y-8 shadow-2xl animate-in zoom-in-95 sticky top-12">
             <div className="flex justify-between items-center border-b border-slate-800 pb-6">
                <div>
                  <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Synthesis Agent</h4>
                  <p className="text-[8px] font-mono text-slate-600 uppercase mt-1">Status: Scrubbing Context...</p>
                </div>
             ...
             <div className="space-y-4">
                <p className="text-[11px] text-slate-400 italic leading-relaxed font-mono">
                  "I've synthesized a version of this insight that retains the strategic logic while ensuring zero exposure of sensitive context."
                </p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-[3.5rem] p-10 space-y-8 shadow-2xl">
             <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.5em] border-b border-slate-800 pb-6">Studio Diagnostics</h4>
             <div className="space-y-8">
                <div className="flex justify-between items-center group">
                   <span className="text-[10px] text-slate-600 uppercase tracking-widest group-hover:text-slate-400 transition-colors">Studio Environment</span>
                   <span className="text-[10px] font-bold text-indigo-400 font-mono">DEPLOYMENT_v7</span>
                </div>
                <div className="flex justify-between items-center group">
                   <span className="text-[10px] text-slate-600 uppercase tracking-widest group-hover:text-slate-400 transition-colors">Persona Mesh</span>
                ...
                <div className="flex justify-between items-center group">
                   <span className="text-[10px] text-slate-600 uppercase tracking-widest group-hover:text-slate-400 transition-colors">Access Level</span>
             ...
             <div className="p-8 bg-slate-950 border border-slate-800 rounded-[2.5rem] space-y-4">
               ...
               <p className="text-[10px] text-slate-600 leading-relaxed font-mono italic">
                 "The Studio allows you to calibrate the twin's logic in real-time, ensuring public responses match your latest professional reasoning."
               </p>
             </div>
          </div>
        )}
