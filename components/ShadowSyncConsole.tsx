...
const ShadowSyncConsole: React.FC<ShadowSyncConsoleProps> = ({ persona }) => {
  ...
  // Files that actually exist in the project and need to be fetched
  const sourceFiles = [
  ...
  // Hardcoded manifests required for Cloud Run build
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setStatus({ type: 'success', msg: 'Sync Complete. Build Triggered.' });
    } catch (e: any) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl space-y-8">
      <div className="flex justify-between items-center border-b border-slate-800 pb-6">
        <div>
          <h3 className="text-sm font-bold text-white uppercase tracking-widest">Cloud Uplink v2</h3>
          <p className="text-[10px] text-slate-500 font-mono uppercase mt-1">Direct DNA to GitHub Pipeline</p>
        </div>
        <div className="flex items-center gap-3">
          <div className={`w-2 h-2 rounded-full ${status.type === 'loading' ? 'bg-blue-500 animate-pulse' : 'bg-slate-700'}`}></div>
          <span className="text-[9px] font-mono text-slate-500">UPLINK_READY</span>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <input 
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 text-xs text-white outline-none focus:border-blue-500 transition-all"
          />
        </div>
        <div className="space-y-4">
      ...
      <div className="space-y-6">
        <button 
          ...
          className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-5 rounded-2xl font-bold text-[10px] uppercase tracking-[0.3em] transition-all shadow-xl"
        >
          {status.type === 'loading' ? 'Uploading Blueprint...' : 'Initialize Cloud Sync'}
        </button>
        ...
        {status.type === 'loading' && (
          <div className="space-y-3">
            ...
            <p className="text-[8px] text-slate-500 font-mono text-center uppercase tracking-widest">
              Pushing: <span className="text-white">{currentFile}</span> ({progress}%)
            </p>
          </div>
        )}
