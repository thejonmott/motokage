...
const OriginStoryView: React.FC<OriginStoryViewProps> = ({ persona, setPersona, accessLevel }) => {
  ...
  const handleSaveFact = () => {
    ...
    if (editingFactId) {
      ...
      setPersona(prev => ({
        ...prev,
        originFacts: [...prev.originFacts, fact].sort((a, b) => parseInt(a.year) - parseInt(b.year))
      }));
    }
  ...
  // --- INTERESTS LOGIC ---
  const handleAddInterest = (category: keyof Persona['interests']) => {
    const name = prompt(`Enter ${category.slice(0, -1)} name:`);
    if (!name) return;
    const meta = prompt(`Additional notes (e.g. Favorite books, albums, or "Why"):`);
    
    if (category === 'hobbies') {
        setPersona(prev => ({ ...prev, interests: { ...prev.interests, hobbies: [...prev.interests.hobbies, name] } }));
    } else {
        const item: InterestItem = { id: `i_${Date.now()}`, name, meta: meta || '' };
        setPersona(prev => ({
            ...prev,
            interests: { ...prev.interests, [category]: [...(prev.interests[category] as InterestItem[]), item] }
        }));
    }
  };

  const deleteInterest = (category: keyof Persona['interests'], id: string) => {
    if (category === 'hobbies') {
  ...
  const timelineWithEras = useMemo(() => {
    ...
    persona.originFacts.forEach(fact => {
      const era = fact.year || 'Epoch Unknown';
    ...
    <div className="space-y-16 animate-in fade-in duration-1000 pb-32 relative text-left">
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 border-b border-slate-900 pb-12">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-3 px-4 py-1.5 bg-emerald-500/10 text-emerald-400 text-[10px] font-bold rounded-full border border-emerald-500/20 uppercase tracking-[0.4em]">Chronology & Legacy Matrix</div>
        ...
        <div className="relative pt-12">
          {!isLocked && (
            <div className="flex justify-end mb-12">
              <button onClick={() => setIsAddingFact(true)} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl hover:bg-indigo-500">Etch New Memory</button>
            </div>
          )}
          ...
          <div className="relative ml-8 md:ml-32 border-l-2 border-slate-900 py-12 space-y-24">
            ...
            {timelineWithEras.map((group, gIdx) => (
              <div key={group.era} className="relative">
                <div className="absolute -left-[41px] top-0 w-20 flex justify-center items-center h-10 z-10">
                   <div className="bg-slate-950 px-4 py-1 border border-slate-800 rounded-full text-[10px] font-bold text-slate-400 uppercase tracking-widest">{group.era}</div>
                </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-6">
                          ...
                          {!isLocked && (
                            <div className="flex gap-4">
                              <button onClick={() => startEditFact(fact)} className="text-slate-500 hover:text-white transition-colors"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                              <button onClick={() => deleteFact(fact.id)} className="text-slate-500 hover:text-rose-500 transition-colors"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
                            </div>
                          )}
                        </div>
                        <p className="text-sm text-slate-400 leading-relaxed font-light mb-6 border-l-2 border-indigo-500/30 pl-6">{fact.significance}</p>
                        {fact.details && (
        ...
        <div className="pt-12 space-y-12">
          {!isLocked && (
            <div className="flex justify-end">
              <button onClick={() => setIsAddingRelation(true)} className="px-8 py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl">Document Relation</button>
            </div>
          )}
          
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {persona.relationships.map(rel => (
              <div key={rel.id} className="bg-slate-900/40 border border-slate-800 p-10 rounded-[2.5rem] space-y-6 hover:border-emerald-500/30 transition-all group relative">
                ...
                <div className="space-y-4">
                   ...
                   {rel.memories && (
                     <div className="pt-4 border-t border-slate-800 space-y-2">
                        <span className="text-[8px] font-bold text-slate-600 uppercase tracking-widest">Core Memories</span>
                        <p className="text-[10px] text-slate-400 italic leading-relaxed">{rel.memories}</p>
                     </div>
                   )}
                </div>
                {!isLocked && (
                  <div className="flex gap-4 pt-4">
                    <button onClick={() => startEditRelation(rel)} className="text-[8px] font-bold text-slate-600 hover:text-white uppercase tracking-widest">Edit</button>
                    <button onClick={() => deleteRelation(rel.id)} className="text-[8px] font-bold text-slate-600 hover:text-rose-500 uppercase tracking-widest">Delete</button>
                  </div>
                )}
              </div>
            ))}
      ...
      {activeSubTab === 'favorites' && (
        <div className="pt-12 grid md:grid-cols-2 gap-12">
          {/* HOBBIES */}
          <div className="bg-slate-900/40 border border-slate-800 p-12 rounded-[3.5rem] space-y-8">
             <div className="flex justify-between items-center">
                <h4 className="text-lg font-bold text-white uppercase tracking-tight font-heading">Hobbies</h4>
                {!isLocked && <button onClick={() => handleAddInterest('hobbies')} className="w-8 h-8 rounded-full border border-slate-800 flex items-center justify-center text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all">+</button>}
             </div>
             <div className="flex flex-wrap gap-3">
                {persona.interests.hobbies.map(h => (
                  <div key={h} className="group px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-3">
                    {h}
                    {!isLocked && <button onClick={() => deleteInterest('hobbies', h)} className="opacity-0 group-hover:opacity-100 text-rose-500">×</button>}
                  </div>
                ))}
             </div>
          </div>
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-12 rounded-[3.5rem] space-y-8">
             <div className="flex justify-between items-center">
                ...
                {!isLocked && <button onClick={() => handleAddInterest('bands')} className="w-8 h-8 rounded-full border border-slate-800 flex items-center justify-center text-purple-400 hover:bg-purple-600 hover:text-white transition-all">+</button>}
             </div>
             <div className="space-y-4">
                {persona.interests.bands.map(b => (
                  <div key={b.id} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl flex justify-between items-start group">
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-12 rounded-[3.5rem] space-y-8">
             <div className="flex justify-between items-center">
                ...
                {!isLocked && <button onClick={() => handleAddInterest('authors')} className="w-8 h-8 rounded-full border border-slate-800 flex items-center justify-center text-cyan-400 hover:bg-cyan-600 hover:text-white transition-all">+</button>}
             </div>
             <div className="space-y-4">
                {persona.interests.authors.map(a => (
                  <div key={a.id} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl flex justify-between items-start group">
                    <div className="space-y-1">
          ...
          <div className="bg-slate-900/40 border border-slate-800 p-12 rounded-[3.5rem] space-y-8">
             <div className="flex justify-between items-center">
                ...
                {!isLocked && <button onClick={() => handleAddInterest('movies')} className="w-8 h-8 rounded-full border border-slate-800 flex items-center justify-center text-amber-400 hover:bg-amber-600 hover:text-white transition-all">+</button>}
             </div>
             <div className="space-y-4">
                {persona.interests.movies.map(m => (
                  <div key={m.id} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl flex justify-between items-start group">
                    <div className="space-y-1">
                      <div className="text-[11px] font-bold text-white uppercase tracking-wider">{m.name}</div>
                      <p className="text-[9px] text-slate-500 font-mono">Significance: <span className="text-slate-400">{m.meta}</span></p>
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[200] flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] w-full max-w-2xl shadow-2xl space-y-8 relative overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-6">
              <input type="text" placeholder="Year (2020)" value={newFact.year} onChange={e => setNewFact({...newFact, year: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-indigo-500" />
              <input type="text" placeholder="Month (January)" value={newFact.month} onChange={e => setNewFact({...newFact, month: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-indigo-500" />
              <input type="text" placeholder="Day (15)" value={newFact.day} onChange={e => setNewFact({...newFact, day: e.target.value})} className="bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-indigo-500" />
            </div>
            <input type="text" placeholder="Event Title" value={newFact.event} onChange={e => setNewFact({...newFact, event: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-indigo-500" />
            <input type="text" placeholder="Brief Significance" value={newFact.significance} onChange={e => setNewFact({...newFact, significance: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-indigo-500" />
            <textarea placeholder="Notes (Autowrapping detailed account...)" value={newFact.details} onChange={e => setNewFact({...newFact, details: e.target.value})} className="w-full h-32 bg-slate-950 border border-slate-800 rounded-2xl p-6 text-[11px] font-mono text-slate-400 outline-none resize-none no-scrollbar" />
            
            <div className="flex gap-4">
              <button onClick={handleSaveFact} className="flex-grow bg-indigo-600 text-white py-5 rounded-2xl font-bold text-[11px] uppercase tracking-widest">{editingFactId ? 'Update Ledger' : 'Confirm Entry'}</button>
              <button onClick={() => { setIsAddingFact(false); setEditingFactId(null); }} className="px-10 py-5 bg-slate-950 text-slate-500 rounded-2xl text-[10px] font-bold uppercase tracking-widest">Cancel</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] w-full max-w-2xl shadow-2xl space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-8 text-left">
              ...
              <div className="space-y-4">
                 ...
                 <input type="text" placeholder="Name" value={newRelation.name} onChange={e => setNewRelation({...newRelation, name: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none focus:border-emerald-500" />
              </div>
            </div>
            
            <div className="grid md:grid-cols-2 gap-8 text-left">
               <div className="space-y-4">
                  <label className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Date of Birth</label>
               ...
               {newRelation.type === 'SPOUSE' && (
                 <div className="space-y-4">
                    ...
                    <input type="text" placeholder="MM/DD/YYYY" value={newRelation.marriageDate} onChange={e => setNewRelation({...newRelation, marriageDate: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-6 py-4 text-xs text-white outline-none" />
                 </div>
               )}
            </div>

            <div className="space-y-4 text-left">
               <label className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Core Memories & Shared History</label>
               <textarea placeholder="Tell the story of how you met or favorite moments..." value={newRelation.memories} onChange={e => setNewRelation({...newRelation, memories: e.target.value})} className="w-full h-32 bg-slate-950 border border-slate-800 rounded-2xl p-6 text-[11px] font-mono text-slate-400 outline-none resize-none no-scrollbar" />
            </div>

            <div className="flex gap-4">
              <button onClick={handleSaveRelation} className="flex-grow bg-emerald-600 text-white py-5 rounded-2xl font-bold text-[11px] uppercase tracking-widest">Commit to Kinship</button>
              <button onClick={() => { setIsAddingRelation(false); setEditingRelationId(null); }} className="px-10 py-5 bg-slate-950 text-slate-500 rounded-2xl text-[10px] font-bold uppercase tracking-widest">Cancel</button>
            </div>
          </div>
        </div>
      )}

      {/* Profile Image with preserved tilt */}
      <style>{`
        @keyframes scan {
          0% { top: 0%; opacity: 0.1; }
          50% { opacity: 0.5; }
